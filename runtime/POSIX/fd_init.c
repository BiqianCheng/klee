...
static void __create_new_dfile(exe_disk_file_t *dfile, unsigned size,
  ...
  if (is_foreign) {
    ...
    klee_assume(/* ss->ss_family != PF_INET  || */ ((struct sockaddr_in *)ss)
                    ->sin_port != 0);
    klee_assume(/* ss->ss_family != PF_INET6 || */ ((struct sockaddr_in6 *)ss)
                    ->sin6_port != 0);
  }
}
...
void klee_init_fds(unsigned n_files, unsigned file_length,
                   ...
                   int save_all_writes_flag, unsigned n_streams,
  ...
  if (__exe_fs.max_failures) {
    ...
    klee_make_symbolic(__exe_fs.read_fail, sizeof(*__exe_fs.read_fail),
                       "read_fail");
    klee_make_symbolic(__exe_fs.write_fail, sizeof(*__exe_fs.write_fail),
                       "write_fail");
    klee_make_symbolic(__exe_fs.close_fail, sizeof(*__exe_fs.close_fail),
                       "close_fail");
    klee_make_symbolic(__exe_fs.ftruncate_fail,
                       sizeof(*__exe_fs.ftruncate_fail), "ftruncate_fail");
    klee_make_symbolic(__exe_fs.getcwd_fail, sizeof(*__exe_fs.getcwd_fail),
                       "getcwd_fail");
  }
