...
int __socketcall(int type, unsigned long *args) {
  ...
  case SYS_ACCEPT:
    return __fd_accept(args[0], (struct sockaddr *)args[1],
                       (socklen_t *)args[2]);
...
#define PORT(addr)                                                             \
  (((struct sockaddr *)(addr))->sa_family == PF_INET                           \
       ? ((struct sockaddr_in *)(addr))->sin_port                              \
   : ((struct sockaddr *)(addr))->sa_family == PF_INET6                        \
...
#define ADDR_COMPATIBLE(addr, addrlen, a)                                      \
  ((addrlen) == (a)->addrlen &&                                                \
   (addr)->sa_family == (a)->addr->ss_family)
...
static exe_disk_file_t *__get_sym_stream() {
  // char msg[64];
  // sprintf(msg, "sym streams used/all %d/%d", __exe_fs.n_sym_streams_used,
  // __exe_fs.n_sym_streams); klee_warning(msg);
...
static exe_sockaddr_t *__allocate_sockaddr(int domain, exe_sockaddr_t *a) {
  assert(a);
  switch (domain) {
#define SET_SOCKADDR(a, type, family)                                          \
  do {                                                                         \
...
int __fd_accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  if (f->dfile) {
    ...
    connf->dfile->src->addr->ss_family =
        connf->local.addr->ss_family;
    ...
    if (addr) {
      ...
      *addrlen =
          connf->foreign
              ->addrlen; /* man page: when addr is NULL, nothing is filled in */
    }
    ...
    // __NR_accept appear to be undecleard, so I change it to 43, which corrspond to accept
    int os_r = syscall(43, f->fd, addr, addrlen);
    if (os_r < 0) {
      errno = klee_get_errno();
...
ssize_t __fd_sendto(int fd, const void *buf, size_t len, int flags,
                    const struct sockaddr *to, socklen_t tolen) {
  struct iovec iov;
  struct msghdr msg;
...
ssize_t __fd_recvfrom(int fd, void *buf, size_t len, int flags,
                      struct sockaddr *from, socklen_t *fromlen) {
  struct iovec iov;
  struct msghdr msg;
  ssize_t s;
  ...
  msg.msg_name = from;
  msg.msg_namelen =
      fromlen ? *fromlen : /* In this case, from should be NULL */ 0;
  msg.msg_iov = &iov;
  msg.msg_iovlen = 1;
  msg.msg_control = NULL;
  msg.msg_controllen = 0;
